...
function machineRuns(machineId: string, recipe: Recipe, nodeById: Map<string, FlowNode>, edges: Edge[]): number {
  let runs = 1;
...
function timeForMachine(
  ...
  const ownTime = recipe?.durationTicks ? machineRuns(machineId, recipe, nodeById, edges) * recipe.durationTicks : 0;
